...
import { SelectProps } from '@/components/functional/interfaces/props/select.interface';
import { SelectFunctionality } from '@/components/functional/interfaces/functions/select.abstract';
import { SelectEvents } from '@/components/functional/interfaces/events/select.abstract';
...
export function selectComposition(options: UseSelectCompositionOptions) {
  const selectedValue = options.selectedValue || ref<string | number | null>(null);
  ...
  const toggleList = () => {
    ...
    const functionality = options.functionality || new CustomSelectFunctionality();  // 구체적인 클래스를 사용
